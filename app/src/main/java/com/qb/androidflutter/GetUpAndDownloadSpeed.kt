...
class GetUpAndDownloadSpeed {
    ...
    fun getTotalUpSpeed() : Float{                    //refreshTime : 刷新时间
        ...
        //计算上传速度
        totalUpSpeed = BigDecimal((mCurrentTotalUp/1024)*1000  /  ((nowTimeStampTotalUp-lastTimeStampTotalUp)*1.0)).setScale(defaultPointAmount,BigDecimal.ROUND_HALF_UP).toFloat() //单位： KB/s
    ...
    fun getTotalDownloadSpeed() : Float{
        ...
        //计算下行速度
        totalDownSpeed = BigDecimal((mCurrentTotalDown/1024)*1000  /  ((nowTimeStampTotalDown-lastTimeStampTotalDown)*1.0)).setScale(defaultPointAmount,BigDecimal.ROUND_HALF_UP).toFloat() //单位： KB/s
    ...
    fun getUpSpeedByUid(uid : Int) : Float{
        ...
        //计算上传速度
        upSpeed = BigDecimal((mCurrentUp/1024)*1000  /  ((nowTimeStampUp-lastTimeStampUp)*1.0)).setScale(defaultPointAmount,BigDecimal.ROUND_HALF_UP).toFloat() //单位： KB/s
        //保存当前的流量总和和上次的时间戳
    ...
    fun getDownloadSpeedByUid(uid: Int) : Float{
        ...
        downSpeed = BigDecimal((mCurrentDown/1024)*1000  /  ((nowTimeStampDown-lastTimeStampDown)*1.0)).setScale(defaultPointAmount,BigDecimal.ROUND_HALF_UP).toFloat() //单位： KB/s
